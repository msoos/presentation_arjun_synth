...
\bigskip
\scriptsize
We try different Manthan strategies, but we won't go into that today. Controlled by \texttt{synth.cpp}
\end{frame}
...
\begin{frame}{Stage 2 --- Backward round: definitions over \emph{anything}}
  Extend only accepts $y = f(X)$. \textbf{Backward}
...
\begin{frame}{Stage 3 --- Unate definitions (two flavours)}
  ...
  \end{exampleblock}
  \medskip
  \scriptsize Equivalence probing has an adaptive disable
  (\texttt{eq\_attempts\_since\_last\_hit}) so we stop wasting SAT calls
...
\begin{frame}{Takeaways}
  \begin{itemize}\setlength{\itemsep}{2pt}
    ...
    \item \textbf{Manthan} (CEGAR) handles the residue: BVE-style initial
          ...
          repair.
    \item \textbf{AIGRewriter + AIGToCNF} are key elements,
