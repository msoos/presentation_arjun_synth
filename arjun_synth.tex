...
\definecolor{stageC}{RGB}{225,140, 43}   % CEGAR
...
\author{Mate Soos, Supratik Chakraborty, Kuldeep S.\ Meel}
\date{\today}
...
\begin{frame}{The synthesis problem}
  ...
  (CNF simplification --- \emph{not} covered here) and then attacks the
  \emph{synthesis} problem on the simplified formula.
\end{frame}

% =========================================================================
\begin{frame}{Three kinds of variables}
  ...
  \emph{to-define} and into \emph{input} or \emph{backward-defined}, with a verified AIG
  attached. When the to-define set is empty we are done.
  \medskip
...
\bigskip
\scriptsize
We try different CEGAR strategies, but we won't go into that here.
\end{frame}

% =========================================================================
\begin{frame}{Stage 1 --- Extend inputs via McMillan interpolant}
  ...
  \textbf{The construction} (also used by
  ...
  (idea: \cite{mcmillan2003interpolation},
...
% =========================================================================
\begin{frame}{Stage 2 --- Backward round: definitions over \emph{anything}}
  Extend only accepts $y = f(X)$. \textbf{Backward} relaxes that:
  $y_i$ may be defined by inputs
  ...
  interpolant only has to talk about $X$). Backward catches the
...
% =========================================================================
\begin{frame}{Stage 3 --- Unate definitions (two flavours)}
  \textbf{Pure unate}.
  ...
  $ y \;=\; L \quad\text{or}\quad y \;=\; \neg L$ is a valid definition.
  \smallskip

  Candidates are ordered: inputs
  ...
  \scriptsize
  \texttt{[unate\_def] units: 1 eq defs: 0 eq calls: 1973
   tested: 146\\
...
% =========================================================================
\begin{frame}{Stage 4 --- The CEGAR core}
  Whatever Stages 1--3 leave is handed to the \textbf{CEGAR} stage: it
  starts with a \emph{guess} for every remaining
  $y_i$ and \alert{repairs} it using counterexamples returned by a SAT solver.
  \medskip

  \textbf{Two SAT solvers, side by side.}
  \begin{itemize}\setlength{\itemsep}{1pt}
    \item \textbf{cex solver} ---
    ...
    \item \textbf{repair solver} --- cached \textbf{CaDiCaL} used to
  ...
  (100 by default in the BVE strategy) and from the BVE-style
  starting functions (next slide).
\end{frame}

% =========================================================================
\begin{frame}{CEGAR's BVE starting guess}
  Before any repair, we need \emph{some} candidate $\hat y_i(X, y_{<i})$
  for every undefined $y_i$. The BVE strategy --- the one we
  ...
  \scriptsize
  \texttt{[cegar] BVE and substitute done. funs: 145 funs/s: 8152.48 T: 0.02 mem: 17.68 MB}
  \end{exampleblock}
\end{frame}
...
\node[sB, text width=72mm] (a) {\textbf{(1)} Ask the cex solver:
...
\node[sB, below=of b, text width=72mm] (c) {\textbf{(3)} Ask the repair solver:
...
\node[sB, below=of c, text width=72mm] (d) {\textbf{(4)} Repair:
...
\node[sB, below=of d, text width=72mm] (e) {\textbf{(5)} Recompute $\hat Y$ on the
   ctx, recompute which variables still need repair, loop.};
...
\end{center}
\vspace{-2mm}
\scriptsize Each loop turn fires periodic stats log lines.
Variable order is fixed up front by a BVE-derived ordering (minimises
back-edges) so that ``earliest wrong'' is well-defined and avoids the
cyclic-repair pitfall.
\end{frame}

% =========================================================================
\begin{frame}{Craig interpolants as a smarter conflict}
  ...
  compose-OR captures many repairs at once.
  \medskip

  \textbf{Implementation:}
  \begin{itemize}\setlength{\itemsep}{1pt}
    \item A fresh CaDiCaL instance per repair, with a McMillan
          interpolation proof tracer attached.
...
\begin{frame}{AIG rewriting and AIG-to-CNF -- why they matter}
  \textbf{The CEGAR hot loop is the SAT solver.} Every repair grows the
  candidate AIG for some $y_i$; every grown AIG must be Tseitin-encoded
  into the cex solver. If we encoded naively, the cex solver
  would balloon and dominate runtime.
  \medskip

  \textbf{AIG rewriting.}
  Each repair round runs a simplification pass + an optional rewrite
  ...
  \textbf{AIG-to-CNF encoding.}  K-ary AND/OR
...
\begin{frame}{A worked example -- \texttt{bobtuint07neg}}
  ...
  \begin{tabular}{@{}lrrr@{}}
    ...
    backward  & 99                  & 146              & 0.07\,s \\
    unate\_def            & 1                   & 145              & 1.02\,s \\
    \midrule
    CEGAR (BVE strategy) & \textbf{145}        & \textbf{0}       & 4.62\,s \\
    \bottomrule
  \end{tabular}
  \end{center}
  \medskip
  ...
  \begin{itemize}
    \item \textbf{Cheap stages first.} 209 of 353 vars defined before
          CEGAR starts --- CEGAR only handles the hard 145.
    \item \textbf{CEGAR stats.} 1864 repairs in 1631 loops; 24\%
          repair-solver cache-hit rate; conflict size avg 8.96 lits.
    \item \textbf{Time breakdown.} 39\% in counterexample search,
          21\% in conflict extraction, 22\% in recomputing candidate values
          --- SAT calls and AIG bookkeeping dominate.
  \end{itemize}
\end{frame}

% =========================================================================
\begin{frame}{Results: Arjun vs Cadet}
        \centering
        \includegraphics[height=220pt]{figures/cdf.pdf}
\end{frame}

% =========================================================================
\begin{frame}{Takeaways}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Synthesis is best done in stages}  From cheap, global, non-iterative
          techniques (e.g. extend, backward) to expensive,
            iterative ones (CEGAR).
    \item Arjun synthesises Boolean functions on a Puura-simplified
        CNF: \textbf{input / to-define / backward-defined} are the key data
          structures
    \item \textbf{CEGAR} handles the residue: BVE-style initial
          guess, CMSGen samples, CryptoMiniSat-backed repair solver,
          conflict (or interpolant) generalisation, AIG composition
          repair.
    \item \textbf{AIG rewriting + AIG-to-CNF} are key elements,
          they keep the cex solver small so the CEGAR loop stays fast.
    \item \textbf{Available for every major OS}:
        Linux, Mac, Windows, compiled for x86, ARM, and WASM.
  \end{itemize}
\end{frame}
