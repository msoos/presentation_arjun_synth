...
\bigskip
\scriptsize
We try different Manthan strategies, but we won't go into that here.
\end{frame}

% =========================================================================
\begin{frame}{Stage 1 --- Extend (UNSAT-define) via McMillan interpolant}
...
\begin{frame}{Stage 3 --- Unate definitions (two flavours)}
  ...
  \end{exampleblock}
  \medskip
  \scriptsize Equivalence probing has an adaptive disable so we stop
  wasting SAT calls on instances where no single-literal definition exists.
\end{frame}

% =========================================================================
\begin{frame}{Stage 5 --- Manthan, the CEGAR core}
  Whatever Stages 0--4 leave is handed to \textbf{Manthan}. Manthan is a
  CEGAR-style synthesiser: it starts with a \emph{guess} for every remaining
...
\begin{frame}{A worked example -- \texttt{bobtuint07neg}}
  ...
  \begin{tabular}{@{}lrrr@{}}
    ...
    \midrule
    Manthan (BVE strategy) & \textbf{145}        & \textbf{0}       & 4.62\,s \\
    \bottomrule
  \end{tabular}
  \end{center}
  \medskip
...
\begin{frame}{Takeaways}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Synthesis is a spectrum.}  From cheap, global, non-iterative
          techniques (e.g. BVE, extend) to expensive,
            iterative ones (Manthan).
