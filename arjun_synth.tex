...
\node[sA, below=of in] (s0)
 {\textbf{0. BVE for synthesis} {\tiny (\texttt{--synthbve}, on by default)}\\
  {\tiny one more BVE pass that is allowed to eliminate \emph{to-define} vars}};

\node[sA, below=of s0] (s1)
...
% =========================================================================
\begin{frame}{Stage 0--1 --- BVE for synthesis, then Autarky}
  \textbf{BVE-for-synthesis.} Same engine as the preprocessor's
  \texttt{occ-bve}, but variables in \emph{to-define} are eligible for
  elimination. Eliminating $y_i$ gives an \alert{exact} AIG definition
  ``for free'': the resolvents we replace with are the conjunction of
  $y_i$'s clauses, which is the function. (\texttt{src/manthan.cpp}'s
  \texttt{bve\_and\_substitute()} reuses the very same idea later for
  initial guesses.)
  \medskip
...
\begin{frame}{Stage 4 --- Unate definitions (two flavours)}
  ...
  \texttt{[unate\_def] units: 1 cond defs: 0 cond calls: 1973
   ...
   Done. defined: 1 still to-define: 145}
  \end{exampleblock}
  \medskip
...
\begin{frame}{Manthan's BVE starting guess}
  ...
  \scriptsize
  \texttt{[manthan] BVE and substitute done. funs: 145 \\
   funs/s: 8152.48 T: 0.02 mem: 17.68 MB}
  \end{exampleblock}
\end{frame}
